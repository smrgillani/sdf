    ...
    <div *ngIf="documentItem" class="document-container">
      <div [ngSwitch]="document.doc_type" class="document-element">
        <div *ngSwitchCase="'document'" class="document">
          <app-text-editor [(ngModel)]="content" [isCollab]="true" (forceSave)="onForceSave()" [readOnly]="readOnly"></app-text-editor>
        </div>
        ...
        <div *ngSwitchCase="'spreadsheet'" class="document">
           <app-spreadsheet [(ngModel)]="content"  (forceSave)="onForceSave()" [isReadOnly]="readOnly"></app-spreadsheet>
        </div>
        ...
        <div *ngSwitchCase="'ocr'" class="document">
          <div class="diagram-container">
            <app-ocr-input [(ngModel)]="content" (forceSave)="onForceSave()" [isWorkArea]="true" [viewOnly]="readOnly"></app-ocr-input>
          </div>
        </div>
...
export class ServiceDocumentComponent implements OnInit {
  ...
  getDocument(id: number, serviceId: number) {
    /*this.orderService.get(id).subscribe((document: DocumentModel) => {
  ...
  createDocument() {
    ...
    this.orderService.createDocument(formData)
      .subscribe((document: DocumentModel) => {
        this.document = document;
        this.document['name'] = this.document['document_name'];
        this.isSaving = false;
      });
  }
  ...
  updateDocument() {
    ...
    this.isSaving = true;
    const formData = this.getFormData();
    this.orderService.saveDocument(formData, this.document.id)
      .subscribe((document: DocumentModel) => {
        this.document = document;
        this.document['name'] = this.document['document_name'];
        this.isSaving = false;
