...
export class CollaborationDocumentComponent implements OnInit, OnDestroy {
    ...
    fileReader.onloadend = (event: ProgressEvent) => {
      if (fileType === 'xlsx' || fileType === 'xls') {
        if (event.target['result']) {
          ...
          self.documentItem = new DocumentItem(this.document);
          self.tasksService.get(this.document.task).subscribe((task) => {
            self.documentItem.parent = new ProcessItem(task);
            self.folderNavigation.addItem(this.documentItem);
            self.folderNavigation.opened.emit(this.documentItem);
            self.uploadDocument = false;
          });
        }
      } else {
  ...
  private getFormData(): FormDataPolyfill {
    ...
    const fileName = `${this.document.name}.${this.document['ext']}`;
    const fileParts = this.document.doc_type === 'ocr' ? [content.content] : [content];
    file = new File(fileParts, fileName);
