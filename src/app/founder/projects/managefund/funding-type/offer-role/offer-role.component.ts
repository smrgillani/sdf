...
export class FounderProjectOfferRoleFunding implements OnInit {
    ...
    manageRoles() {
        ...
        this.companyRoles.forEach((item) => {
            ...
            else if(this.fundData.company_shares.find(f => f.role == item.id)){
                this.fundData.company_shares.find(f => f.role == item.id).roleString = item.title;
            }
        });
    }
