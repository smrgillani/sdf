...
@Component({
  templateUrl: './dynamic-kanban-board.component.html',
  styleUrls: ['./dynamic-kanban-board.component.scss'],
...
export class DynamicKanbanBoardComponent {
  ...
  itemRenderer = (element: any, item: any, resource: any): void => {
    ...
    for (let i = 0; i < tasks.length; i++) {
      if (tasks[i].id.toString() === item.id) {
    ...
    if (tasks_itemid_originid == null) {
      tasks_itemid_originid = [];
    }
    ...
    element[0]
      .getElementsByClassName('jqx-kanban-item-color-status')[0]
      .innerHTML = '<span style="line-height: 23px; ' + 'margin-left: 5px;">' + resource.name + '</span>';
                  ...
                  if (arr.length === response.subtasks.length) {
                    for (let i = 0; i < arr.length; i++) {
                      ...
                      if (container.children.length > 0) {
                        container.insertBefore(newSlideItem, container.children[i]);
                      } else {
                        container.appendChild(newSlideItem);
                      }

                      const processId = arr[i].id;
    ...
    /* touchstart for IOS */
    edit_btn.addEventListener('touchstart', (event: any): void => {
      window.location.href += `/editgoal/${item.resourceId}`;
    });
  };
